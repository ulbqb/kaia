...
func (self *worker) commitNewWork() {
	...
	if self.nodetype == common.CONSENSUSNODE {
		...
		// process with gasless txs only if lend tx and gasless tx exist.
...
func filterGaslessTransaction(pending map[common.Address]types.Transactions) map[common.Address]types.Transactions {
		...
		if set[0] == nil || set[2] == nil {
			for _, s := range set {
				if s != nil {
					...
					for _, tx := range pending[s.addr] {
						if bytes.Equal(tx.Hash().Bytes(), s.hash.Bytes()) {
							continue
						}
						txs = append(txs, tx)
					}
