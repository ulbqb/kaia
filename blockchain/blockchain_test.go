...
func TestAccessListTx(t *testing.T) {
	...
	insertBlockWithTx := func(list types.AccessList) *types.Block {
		...
		blocks, _ := GenerateChain(gspec.Config, block, engine, db, 1, func(i int, b *BlockGen) {
			...
			tx, _ := types.SignTx(types.NewMessage(senderAddr, &contractAddr, senderNonce, big.NewInt(0), 30000, nil, nil, big.NewInt(1), []byte{}, false, intrinsicGas, list, nil), signer, senderKey)
			b.AddTx(tx)
		})
